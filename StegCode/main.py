...
def encode_steganography(binary_string, message, L, S, header):
    if len(message)*8*L + header + S > len(binary_string):
...
def decode_steganography(filename, L, S, header):
    if(header == 8):
    ...
    while choice != 4:
        ...
        if choice == '1':
            while choice != 3:
                ...
                if choice == '1':
                    image_path, S, L, message = input_vars("image")
                    binary_string, width, height = image_init(image_path)
                    binary_image = encode_steganography(binary_string, message, L, S, 8) #Perform steganography encoding
            ...
            while choice != 3:
                ...
                if choice == '1':
                    video_path, S, L, message = input_vars("video")
                    binary_string = video_init(video_path)
                    binary_image = encode_steganography(binary_string, message, L, S, 1280)  # Perform steganography encoding. we skip 10KB for header, arbitrary but safe value
                    decimal_image_data = bytes_sep(binary_image)
            ...
            while choice != 3:
                ...
                if choice == '1':
                    audio_path, S, L, message = input_vars("audio")

                    binary_string, params= audio_init(audio_path)

                    binary_image = encode_steganography(binary_string, message, L, S,44)  # Perform steganography encoding
                    decimal_image_data = bytes_sep(binary_image)
                    try:
